...
1. Compile
   ...
   rm -rf builddir/; \
       meson compile -C builddir/ --clean; \
       PKG_CONFIG_PATH=/usr/lib/i386-linux-gnu/pkgconfig:$PKG_CONFIG_PATH meson setup --cross-file cross -Dprefix=/home/user/Desktop/tmp-mesa/mesa/built -Dgallium-drivers=iris -Dvulkan-drivers= -Dbuildtype=release --wipe builddir/ && \
       meson compile -j 6 -C builddir/
   ```
